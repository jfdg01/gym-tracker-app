...
All user-facing text must be internationalized using the system provided in `src/i18n`. Hardcoded strings are not allowed in UI components.
...
All data operations must be performed through the centralized service layer located in `src/services`.

### Core Services
*   **`exerciseService`**: Management of exercises.
*   **`programService`**: Management of workout programs.
*   **`workoutService`**: Management of active workout sessions.
...
### Rules
1.  **No Direct Repository Access**: UI components must never import or use Repositories directly.
2.  **Service Imports**: Import instances directly from `src/services`.
...
For detailed architectural guidelines, including Safe Area handling, State Management, and Project Structure, please refer to [Architecture Guidelines](./architecture_guidelines.md).